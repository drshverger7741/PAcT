...
1. Проверьте файл `app.log` в папке данных приложения. Туда записываются все сообщения консоли и ошибки.
2. База данных с активностью хранится в файле `pact_db.db` в папке данных приложения.

### Расположение данных
- **Скомпилированный EXE:** `%LOCALAPPDATA%\PAcT\pact_db.db` и `%LOCALAPPDATA%\PAcT\app.log`.
- **Запуск из исходного кода:** `pact_db.db` и `app.log` находятся в папке проекта рядом с `main.py`.
